...
dat <- haven::read_dta(file = dt_path)     # read_dta(dt_path)
...
lbl <- latex2exp::TeX("$X|x=c_0$")

p0 <- ggplot() +
  geom_point(aes(x=X, y=Y, color = as.factor(D)), data = dt_head) +
  geom_vline(xintercept = c, color = "red", lty="dashed", alpha = 0.2) +
  geom_text(aes(x=c+1 , y =20), 
            label= lbl 
            ) 
...
lmfit <- lm(formula = mod_equiv, data = dt_hd)
...
xmerit::lx.est(lm.mod = mod_equiv,
               lm.dt = dt_hd,lm.n = 2,inf = c("over","fit","Ftest"))
